...
function Certifications() {
  const data = useSelector((state) => state.template1.data.certifications);
  const validations = useSelector(
    (state) => state.template1.validations.certifications
  );
  ...
  const addData = () => {
    dispatch(addCertificationAction({ ...localData }));
    setLocalData({
      name: "",
      link: "",
    });
  ...
  const deleteData = (index) => {
    dispatch(deleteCertificationAction(index));
  };
  ...
  return (
    <div>
      <p className="text-xl font-medium text-blue-700">Certifications</p>
